...
def impute_power_output(df: pd.DataFrame, 
                                similar_turbines_df: pd.DataFrame,
    ...
    similar_lookup = (similar_turbines_df
                     .nsmallest(n_similar, 'rank', keep='all')
                     .groupby('turbine_id')['similar_turbine_id']
                     .agg(list)
                     .to_dict())
    
    # Create a pivot table of power values
    power_pivot = df.pivot(index='timestamp', 
                          columns='TurbID', 
                          values='Patv')
    
    # Create a pivot table of imputation flags
    impute_pivot = df.pivot(index='timestamp', 
                           columns='TurbID', 
                           values='impute_day_patv')
    ...
    for turbine_id in turbines_to_impute:
        ...
        power_pivot.loc[timestamps_to_impute, turbine_id] = (
            imputed_values.loc[timestamps_to_impute]
        )
    
    # Convert pivot table back to original format
    df_imputed = (power_pivot
                  .reset_index()
                  .melt(id_vars=['timestamp'], 
                       var_name='TurbID', 
                       value_name='Patv_imputed'))
