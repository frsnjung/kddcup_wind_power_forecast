...
def test_non_imputed_values_unchanged(sample_data, similar_turbines_data):
    ...
    pd.testing.assert_series_equal(
        result_df.loc[mask, 'Patv'],
        sample_data.loc[mask, 'Patv'],
        check_names=False
    )
...
def test_imputed_values_correct(sample_data, similar_turbines_data):
    ...
    mask = ((sample_data['TurbID'] == 3) & 
            (sample_data['timestamp'] == '2021-01-01 00:00'))
    similar_mask = ((sample_data['TurbID'].isin([1, 4, 2])) & 
                   (sample_data['timestamp'] == '2021-01-01 00:00'))
